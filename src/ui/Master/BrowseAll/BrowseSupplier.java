 ...
 * @author Admin
 */
public class BrowseSupplier extends javax.swing.JDialog {
    private SupplierDAO supplierDAO;
    private DefaultTableModel tableModel;
    private List<Supplier> supplierList;
    private List<Supplier> currentItemList = new ArrayList<>();
    private int displayLimit = 10;
    private int currentPage = 1;
    private int totalPages = 1;

    /**
     * Creates new form BrowseSupplier
     */
    public BrowseSupplier(java.awt.Frame parent, boolean modal, Connection conn) {
        super(parent, modal);
        initComponents();
    ...
    private void setupTable() {
        String[] columnNames = {
            "No", "IDSupplier", "Kode", "Nama", "Email", "Alamat", 
    ...
    private void applyDisplayLimit() {
        totalPages = (int) Math.ceil((double) supplierList.size() / displayLimit);
    ...
    private void updatePageData() {
        ...
        int endIndex = Math.min(startIndex + displayLimit, supplierList.size());
        currentItemList = supplierList.subList(startIndex, endIndex);
    ...
    private void updateTable(List<Supplier> list) {
        ...
        for (Supplier s : list) {
            tableModel.addRow(new Object[]{
                ...
                s.getNomorRekening(),
                s.getNamaRekening(),
                (s.getAktif() == 1) ? "Aktif" : "Tidak Aktif"
            });
        }
    }
    ...
    private void selectRowAndClose() {
        ...
        if (selectedRow >= 0) {
            try {
                ...
                if (getParent() instanceof frmMstSupplier) {
                    frmMstSupplier parentForm = (frmMstSupplier) getParent();
                    parentForm.setSupplierData(
                        id, kode, nama, email,
                    ...
                    parentForm.setSupplierData(id, nama);
                }
    ...
    private void searchSupplier() {
        ...
        if (keyword.isEmpty()) {
            ...
            supplierList = supplierDAO.searchSupplierByName(keyword);
            updateTable(supplierList);
        }
    }

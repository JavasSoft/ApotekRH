 ...
 *
 * @author Admin
 */public class frmMstSupplier extends javax.swing.JFrame {
    ...
    /**
     * Creates new form frmMstSupplier
     */
    public frmMstSupplier() {
        initComponents();
        initializeDatabase();
    ...
    private void IDotomatis() {
        try {
            String lastCode = supplierDAO.getLastKode("SP"); // prefix diganti SP
            String newCode;

            if (lastCode != null && lastCode.startsWith("SP")) {
                int number = Integer.parseInt(lastCode.substring(2));
                number++;
                newCode = String.format("SP%03d", number);
            } else {
                newCode = "SP001";
            }
    ...
    private void awal() {
        ...
        totalInputs = supplierDAO.countSupplier();

        if (totalInputs > 0) {
            currentRecordIndex = totalInputs - 1;
            loadCurrentSupplier();
            updateRecordLabel();
        }
        navaktif();
    ...
    private void saveSupplier() {
        ...
        supplier.setNomorRekening(nomorRekening);
        supplier.setNamaRekening(namaRekening);
    ...
    private void updateSupplier() {
        ...
        if (kode.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Kode Supplier harus diisi untuk melakukan update.");
            return;
        }
        ...
        if (supplierDAO.updateSupplier(id, supplier)) {
            ...
            JOptionPane.showMessageDialog(this, "Gagal memperbarui data Supplier.");
        }
    }
    ...
    private void loadDataFromDatabase() {
        ...
        if (supplierList.isEmpty()) {
            JOptionPane.showMessageDialog(null, "Data supplier belum ada di database.");
        } else {
    ...
    private void loadCurrentSupplier() {
        if (currentRecordIndex >= 0 && currentRecordIndex < supplierList.size()) {
            ...
            txtIDSupplier.setText(String.valueOf(supplier.getIDSupplier()));
            ...
            txtNomorRekening.setText(supplier.getNomorRekening());
            txtNamaRekening.setText(supplier.getNamaRekening());
            cmbAktif.setSelected(supplier.getAktif() == 1);
    ...
    private void next() {
        if (currentRecordIndex < totalInputs - 1) {
            currentRecordIndex++;
            loadCurrentSupplier();
        } else {
